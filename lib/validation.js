    ...
    async.parallel(
      [
        function(callback){ // VALIDATE API OPTIONS
          ...
          var optionsValidation = templates.validate('options', data.apiOptions); // VALIDATE INPUT BODY USING JSON SCHEMA
          if (!optionsValidation) {
            //console.log('VALIDATION >> resourcesValidation success: ' + JSON.stringify(resourcesValidation) + '\n');
            callback(null, optionsValidation);
          } else {
            console.log('VALIDATION ERROR >> optionsValidation: ' + JSON.stringify(optionsValidation) + '\n');
            err = {
              "name" : "Options",
              "code" : 400,
              "type" : "Validation",
              "value" : "Missing or Invalid API Name"
            };
            callback(err, null);
          }
        },
        ...
        function(callback){ // VALIDATE BASIC API MODEL
          ...
          var basicModelsValidation = templates.validate('basicModels', data.apiModels); // VALIDATE INPUT BODY USING JSON SCHEMA
          if (!basicModelsValidation) {
            callback(null, basicModelsValidation);
          } else {
            console.log('VALIDATION ERROR >> basic models Validation: ' + JSON.stringify(basicModelsValidation) + '\n');
            err = {
              "name" : "Models",
              "code" : 400,
              "type" : "Validation",
  ...
  this.replaceRequiredProperties = function(data, callback){
    ...
    async.each(data.apiModels, function(model, eachCallback) {
      ...
      recurseReplaceRequiredProperties(model, eachCallback);
        ...
        if( err ) {
          err = {
            "name" : "Required Properties Error",
            "code" : 400,
            "type" : "Validation",
            "value" : "There was a problem processing your interface. Please try again."
          };
          callback(err);
        } else {
