...
*/
function recurseWriteViewsOutput(resourceObject, outputFile, propertyKeyParent, callback){
  ...
  async.whilst(
    ...
    function(cb) {
      ...
      if(resourceObject[field].type === 'object'){
        ...
        recurseWriteViewsOutput(resourceObject[field].properties, outputFile, propKeyParent, cb);
      } else {
    ...
    function(cb) {
      ...
      async.series(
        [
          ...
          function(callback_inner){
            outputFile.write('\t"_links" : {\n');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t"self": { "href": "{{path}}" },\n');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t{{#if parent}}"parent" : { "href" : "{{parent}}" },{{/if}}');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t{{#if children}}');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t\t{{#each children}}');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t\t\t"{{this}}" : { "href" : "{{path}}{{this}}" },');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
          ...
          function(callback_inner){
            outputFile.write('\t\t"curies": [{ "name": "' + resource.name + '", "href": "http://example.com/docs/rels/' + resource.name + '", "templated": false }],\n');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t{{#if collection}}"next": { "href": "{{path}}?offset=50&limit=50" }{{/if}}\n');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t},\n');
          ...
          function(callback_inner){
            outputFile.write('\t{{#if collection}}\n');
            callback_inner(null,{"line2" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t{{#each ' + resource.name + '}}\n\t\t\t');
            callback_inner(null,{"line3" : true});
          },
          function(callback_inner){
            recurseWriteViewsOutput(resourceModel, outputFile, null, callback_inner);
          },
          function(callback_inner){
            outputFile.write('{{#unless @last}}');
            callback_inner(null,{"line6" : true});
          },
          function(callback_inner){
            outputFile.write(',{{/unless}}\n');
            callback_inner(null,{"line8" : true});
          },
          function(callback_inner){
            outputFile.write('\t\t{{/each}}\n');
...
function writeMeta(transformedData, templateDetails, callback){
  ...
  fs.readFile('./templates/meta.handlebars', 'utf8', function (err,fileData) {
    ...
    var template = _handlebars.compile(fileData);
    var completeTemplate = template(transformedData.lineageArrays.resourceObjects);
    ...
    async.series(
      [
        function(callback){ // EXECUTE TERMINAL COMMANDS
          var commands = [];
          var makeDirectoryCommand = 'mkdir -p generated/' + transformedData.data.apiOptions.apiName + '/{models,routes/{' + transformedData.lineageArrays.lineageArray.join() + '},shared,templates,views}';
