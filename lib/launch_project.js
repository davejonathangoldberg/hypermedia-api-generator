...
function pollApiTest(data, callback){
  var appResponse;
  var options = {
  ...
  var req = https.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if((res.statusCode === 200) || (res.statusCode === 201) || (res.statusCode === 202)){
        console.log('app response equal to 200, 201, 202');
...
function notifySuccess(webhookUrlDomain, appInfo, callback){
  var notifyHookResponse;
  var webhookHost = url.parse(webhookUrlDomain).host;
  var webhookPath = url.parse(webhookUrlDomain).path;
  var options = {
    hostname: webhookHost,
    path: webhookPath + '/' + appInfo.apiId + '/' + appInfo.webhookUrl,
    method: 'POST',
    headers: {
  ...
  var req = https.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if((res.statusCode === 200) || (res.statusCode === 201) || (res.statusCode === 202)){
        console.log('Hook response equal to 200, 201, 202');
                        ...
                        getApiInfo(apiInput, function(err, apiInfo){
                          if(err){
                            ...
                            if(apiInfo.status == 'pending'){
                              pollAppTest(appInput, function(err, appInfo){
                                if(err){
                                  intervalCounter++;
                                  innerParallelCallback(err);
                                } else {
                                  notifySuccess(webhookUrlDomain, appInfo, innerParallelCallback);
                                }
                              });                              
                            } else if(apiInfo.status == 'active'){
                              clearInterval(intervalObject);
                              innerParallelCallback(null, 'pollWebhook');
                            } else {
                              clearInterval(intervalObject);
                              innerParallelCallback('statusError');
                            }
                          }
                        });
                      } else {
                        clearInterval(intervalObject);
                        innerParallelCallback('timeout');
                      }
                    }, delay);
