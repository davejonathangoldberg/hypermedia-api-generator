...
function notifySuccess(webhookUrlDomain, appInfo, callback){
  ...
  console.log('\n\nNOTIFY SUCCESS appInfo.webhookUrl: ' + appInfo.webhookUrl);
  var options = {
              ...
              async.parallel(
                [
                  ...
                  function(innerParallelCallback){ // POLL FOR SUCCESS IN CASE OF WEB HOOK FAIL
                    ...
                    var apiInput = { "apiId" : transformedData.data.apiId, "appId" : newAppDetailsResponse.id };
                            ...
                            if(apiInfo.status == 'pending'){
                              pollAppTest(apiInput, function(err, appInfo){
                                if(err){
                                  ...
                                  appInfo.webhookUrl = encodeURIComponent(transformedData.data.apiOptions.webhookUrl);
                                  notifySuccess(webhookUrlDomain, appInfo, function(){
                                    console.log("NOTIFY SUCCESS CALLBACK CALLED.");
                                  });
                                }
                              });                              
