module.exports = function TopicsSpeakersRoutes(app) {
  ...
  //  RESOURCE VARIABLES
  var resourceName = 'speakers';
  var parentResourceName = 'topics';
  var resourcesLength = 2;
  var lineage = 'topicsspeakers';
  var lineageArray = ['topics','speakers'];
  ...
  app.get('/topics/:topicsInstanceId/speakers', function(req, res, next){
    var parentPath = '';
  ...
  app.post('/topics/:topicsInstanceId/speakers', function(req, res, next){      
    var parentPath = '';
    var instancesArray = [];
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
  ...
  app.get('/topics/:topicsInstanceId/speakers/:speakersInstanceId', function(req, res, next){      
    var parentPath = '';
    var query = {};
    var queryOptions = {};
    if (parentResourceName !== ''){
  ...
  app.put('/topics/:topicsInstanceId/speakers/:speakersInstanceId', function(req, res, next){      
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
  ...
  app.del('/topics/:topicsInstanceId/speakers/:speakersInstanceId', function(req, res, next){ // ONLY USED IN CASES WHERE THE SUB-INSTANCE IS PART OF A SEPARATE PARENT INSTANCE
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
