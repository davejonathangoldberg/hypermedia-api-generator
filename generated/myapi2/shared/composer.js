module.exports = function Composer(app) {
  ...
  // META
  var meta = {};
  meta['posts'] = {'children' : ['tags', 'submarines'], 'stubs' : ['blob']};
  meta['tags'] = {'children' : [], 'parents' : ['posts'], 'stubs' : []};
  meta['submarines'] = {'children' : [], 'parents' : ['posts'], 'stubs' : []};
  meta['widgets'] = {'children' : [], 'parents' : ['submarines'], 'stubs' : []};
  ...
  this.createChildInstance = function(req, res, next, body, resourceName, parentResourceName, parentPath){
    ...
    async.waterfall(
      [
        ...
        function(results, callback){
          if(results.instance){ // CHECKS PARENT PATH. IF DOESN'T EXIST THEN 404, ELSE CREATE NEW RESOURCE. 
            body.createdDate = new Date();
            body.modifiedDate = new Date();
            var data = {};
            data.model = model;
  ...
  this.deleteInstance = function(req, res, resourceName){
    ...
    async.series(
      [
        function(callback){
          if (meta[resourceName].children.length > 0) {
            var data = [];
            var query = {};
            query[resourceName] = req.params[resourceName + 'InstanceId'];
            var updateQuery = { '$pull' : query };
            for (var i = 0; i < meta[resourceName].children.length; i++){
              data[i] = {};
              var childResource = meta[resourceName].children[i];
    ...
    async.waterfall(
      [
        ...
        function(results, callback){ // UPDATES EXISTING RESOURCE, OR IF OPTION UPSERT IS SET TO TRUE, INSERTS NEW RESOURCE WITH A NEW PARENT RESOURCES STRING.
          ...
          data.query = {};
          data.options = options;
          if(results.instance && data.options.upsert && (results.instance[parentResourceName + '_'].indexOf(parentPath) === -1 )) { // IF THE CHILD RESOURCE EXISTS BUT NOT UNDER PARENT PATH, INSERT IT .
            var pushQuery = {};
          ...
          } else if(results.instance && (results.instance[parentResourceName + '_'].indexOf(parentPath) > -1 )) { // RESOURCE EXISTS UNDER THE PARENT PATH
            data.statusCode = 200;
            data.headerObject = {};
            data.query['id'] = req.params[resourceName + 'InstanceId'];
            data.updateQuery = {};
