module.exports = function ItemsAssigneesRoutes(app) {
  ...
  //  RESOURCE VARIABLES
  var resourceName = 'assignees';
  var parentResourceName = 'items';
  var resourcesLength = 2;
  var lineage = 'itemsassignees';
  var lineageArray = ['items','assignees'];
  ...
  app.get('/items/:itemsInstanceId}/assignees', function(req, res, next){
    var parentPath = '';
  ...
  app.post('/items/:itemsInstanceId}/assignees', function(req, res, next){      
    var parentPath = '';
    var instancesArray = [];
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
  ...
  app.get('/items/:itemsInstanceId}/assignees/:assigneesInstanceId', function(req, res, next){      
    var parentPath = '';
    var query = {};
    var queryOptions = {};
    if (parentResourceName !== ''){
  ...
  app.put('/items/:itemsInstanceId}/assignees/:assigneesInstanceId', function(req, res, next){      
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
  ...
  app.del('/items/:itemsInstanceId}/assignees/:assigneesInstanceId', function(req, res, next){ // ONLY USED IN CASES WHERE THE SUB-INSTANCE IS PART OF A SEPARATE PARENT INSTANCE
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
