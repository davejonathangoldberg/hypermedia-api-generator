module.exports = function Family-membersRoutes(app) {
  ...
  //  RESOURCE VARIABLES
  var resourceName = 'family-members';
  var parentResourceName = '';
  var resourcesLength = 1;
  var lineage = 'family-members';
  var lineageArray = ['family-members'];
  ...
  app.get('/family-members', function(req, res, next){
    var parentPath = '';
  ...
  app.post('/family-members', function(req, res, next){      
    var parentPath = '';
    var instancesArray = [];
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
  ...
  app.get('/family-members/:family-membersInstanceId', function(req, res, next){      
    var parentPath = '';
    var query = {};
    var queryOptions = {};
    if (parentResourceName !== ''){
  ...
  app.put('/family-members/:family-membersInstanceId', function(req, res, next){      
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
  ...
  app.del('/family-members/:family-membersInstanceId', function(req, res, next){ // ONLY USED IN CASES WHERE THE SUB-INSTANCE IS PART OF A SEPARATE PARENT INSTANCE
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
