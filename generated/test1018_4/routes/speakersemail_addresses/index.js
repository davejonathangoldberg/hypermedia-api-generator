module.exports = function SpeakersEmail_addressesRoutes(app) {
  ...
  //  RESOURCE VARIABLES
  var resourceName = 'email_addresses';
  var parentResourceName = 'speakers';
  var resourcesLength = 2;
  var lineage = 'speakersemail_addresses';
  var lineageArray = ['speakers','email_addresses'];
  ...
  app.get('/speakers/:speakersInstanceId/email_addresses', function(req, res, next){
    var parentPath = '';
  ...
  app.post('/speakers/:speakersInstanceId/email_addresses', function(req, res, next){      
    var parentPath = '';
    var instancesArray = [];
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
  ...
  app.get('/speakers/:speakersInstanceId/email_addresses/:email_addressesInstanceId', function(req, res, next){      
    var parentPath = '';
    var query = {};
    var queryOptions = {};
    if (parentResourceName !== ''){
  ...
  app.put('/speakers/:speakersInstanceId/email_addresses/:email_addressesInstanceId', function(req, res, next){      
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
  ...
  app.del('/speakers/:speakersInstanceId/email_addresses/:email_addressesInstanceId', function(req, res, next){ // ONLY USED IN CASES WHERE THE SUB-INSTANCE IS PART OF A SEPARATE PARENT INSTANCE
    var parentPath = '';
    if (parentResourceName !== ''){
      for(var i=0; i < resourcesLength-1; i++){
        if(i == 0){
